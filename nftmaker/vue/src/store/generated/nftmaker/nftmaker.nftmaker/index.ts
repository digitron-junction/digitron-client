...
import { Post } from "./module/types/nftmaker/nft"
import { Params } from "./module/types/nftmaker/params"
...
export default {
	...
	actions: {
		init({ dispatch, rootGetters }) {
			console.log('Vuex module: nftmaker.nftmaker initialized!')
		...
		async QueryNfts({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const queryClient=await initQueryClient(rootGetters)
				let value= (await queryClient.queryNfts()).data
	...
	gas: "200000" }, memo})
				...
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgCreateNft:Init Could not initialize signing client. Wallet is required.')
				}else{
					throw new Error('TxClient:MsgCreateNft:Send Could not broadcast Tx: '+ e.message)
				}
			}
		},
		...
		async MsgCreateNft({ rootGetters }, { value }) {
			try {
				...
				if (e == MissingWalletError) {
					...
					throw new Error('TxClient:MsgCreateNft:Create Could not create message: ' + e.message)
				}
			}
		},
